    ...
    <StudioPageState error={query.error} isPending={query.isPending}>
      <div className="mx-auto w-full max-w-6xl overflow-auto py-6">
        <StudioPageHeader
          ...
          action={<StudioLink className="rounded border border-theme-outline px-4 py-2 text-sm text-theme-accent" href="/extensions">打开 OpenBB 安装器</StudioLink>}
        />
        <section className="mt-6 rounded border border-theme-outline bg-theme-primary p-5" aria-labelledby="custom-provider-heading">
          ...
          <StudioLink className="mt-4 inline-flex rounded bg-theme-accent px-4 py-2 text-sm text-theme-primary-inverse" href="/extensions">添加自定义 Provider</StudioLink>
        </section>
