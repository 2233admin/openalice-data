...
export function PlaygroundPage() {
  ...
  const dataset = query.data?.snapshot.datasets.find((item) => item.id === datasetId);
  const availableProviders = dataset?.providers ?? [];
  const provider = providerId;
  const hasTarget = Boolean(dataset && provider && availableProviders.some((item) => item.provider_id === provider));
  ...
  return <StudioPageState error={query.error} isPending={query.isPending}>
    <div className="mx-auto w-full max-w-7xl overflow-auto py-6">
      <StudioPageHeader title="查询" description="这里只运行已明确选择的数据源。目标、Provider、参数和结果会一直保留，失败时直接回到对应的系统维护或数据源详情。" />
      {!hasTarget ? (
        <section className="mt-5 rounded border border-theme-outline bg-theme-primary p-8 text-center" role="status">
          <h2 className="font-semibold">请先选择数据源</h2>
          <p className="mt-2 text-sm text-theme-muted">查询不会默认使用第一个数据集。请从数据源表打开一个明确的 Provider / 数据集目标。</p>
          <StudioLink className="mt-4 inline-block text-sm text-theme-accent" href="/data-sources">返回数据源</StudioLink>
        </section>
      ) : (
        <div className="mt-5 grid grid-cols-[20rem_minmax(0,1fr)] gap-5">
          <form className="rounded border border-theme-outline bg-theme-primary p-5" onSubmit={(event) => void run(event)}>
            <p className="mb-4 text-xs text-theme-muted">目标：{dataset?.display_name} · {provider}</p>
            <label className="block text-sm">数据集<select className="mt-1 w-full rounded border border-theme-outline bg-theme-secondary p-2" onChange={(event) => { setDatasetId(event.target.value); setProviderId(""); setResult(undefined); setFailure(undefined); reset(); }} value={datasetId}><option value="">请选择数据集</option>{query.data?.snapshot.datasets.map((item) => <option key={item.id} value={item.id}>{item.display_name} · {item.category}</option>)}</select></label>
            <label className="mt-4 block text-sm">数据源<select className="mt-1 w-full rounded border border-theme-outline bg-theme-secondary p-2" onChange={(event) => { setProviderId(event.target.value); setResult(undefined); setFailure(undefined); }} value={providerId}><option value="">请选择 Provider</option>{availableProviders.map((item) => <option key={item.provider_id} value={item.provider_id}>{item.provider_id} · {item.state}</option>)}</select></label>
            ...
            {query.data?.service.state !== "running" && <StudioLink className="mt-3 block text-center text-sm text-theme-accent" href="/advanced?section=runtimes">打开系统维护 →</StudioLink>}
          </form>
...
export const Route = createFileRoute("/playground")({
  validateSearch: z.object({ dataset: z.string().optional(), provider: z.string().optional() }),
